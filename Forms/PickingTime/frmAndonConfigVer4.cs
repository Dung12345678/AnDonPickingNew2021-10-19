...
namespace BMS
{
	...
	public partial class frmAndonConfigVer4 : Form
	{
		...
		{
			object takt = TextUtils.ExcuteScalar("Select Takt from SumitomoHyp.dbo.AndonConfig with(nolock)");
			txtTakt.Text = TextUtils.ToString(takt);
			DataTable data = TextUtils.Select("Select * from SumitomoHyp.dbo.Shift with(nolock)");
		...
		{
			object takt = TextUtils.ExcuteScalar("Select top 1 Takt from SumitomoHyp.dbo.AndonConfig");
			txtTakt.Text = TextUtils.ToString(takt);
		...
		private void btnDelete_Click(object sender, EventArgs e)
		{
			...
			try
			{
				...
				TextUtils.ExcuteSQL($"DELETE SumitomoHyp.dbo.Andon WHERE ID={ID}");
				LoadData();
			}
			catch (Exception)
			{
			...
			try
			{
				...
				if (checkValid(dateTimes[0], dateTimes[1], dateTimes[2], dateTimes[3], dateTimes[4], dateTimes[5], dateTimes[6], dateTimes[7], dateTimes[8], dateTimes[9]))
				{
					...
					if (!_isAdd)
					{
						...
						andon = AndonBO.Instance.FindByPKHyp(ID) as AndonModel;
					}
					...
					{
						andon.IsStart = false;
						AndonBO.Instance.InsertHyp(andon);
					}
					else
					{
						//		_UpdateAndon(false);
						AndonBO.Instance.UpdateHyp(andon);
						//	_UpdateAndon(true);
					}
					LoadData();
					SetInterface(false);
		...
		private void frmAndonConfigVer4_FormClosed(object sender, FormClosedEventArgs e)
		{
