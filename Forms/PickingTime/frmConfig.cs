...
namespace BMS
{
	public partial class frmConfig : Form
	{
		...
		{
			ArrayList arr = AndonPickingConfigBO.Instance.FindAll();		
			if (arr.Count > 0)
			{
				AndonPickingConfigModel andonConfig = (AndonPickingConfigModel)arr[0];
		...
		private bool checkInput(string tcp, string port, string com, string areaDelay, string areaRisk)
		{	
			if (tcp == "")
			{
		...
		{
			// check đã nhập chưa
			bool checkPass = checkInput(txtTcpIp.Text, txtPort.Text, txtCom.Text, txtAreaDelay.Text, txtAreaRisk.Text);
			if (!checkPass)
			{
				return;
			}
			...
			if (arr.Count > 0)
			{
				...
				AndonPickingConfigBO.Instance.Update(andonConfig);
			...
			else
			{
				...
				andonConfig.AreaDelayPLC = TextUtils.ToString(txtAreaDelay.Text);
				andonConfig.AreaRiskPLC = TextUtils.ToString(txtAreaRisk.Text);
				AndonPickingConfigBO.Instance.Insert(andonConfig);
				MessageBox.Show("Config font size successfully! ");
			}
		}
		...
		private void btnDeleteAutoAddXK_Click(object sender, EventArgs e)
		{
			try
			{
				TextUtils.ExcuteSQL($"DELETE [ShiStock].[dbo].[AddAutoXKNew] WHERE OrderCode='{txtOrder.Text.Trim()}'");
